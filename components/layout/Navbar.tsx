        ...
        {texts.length > 0 && (
          <div ref={coursesRef} className="relative">
            <button
              ...
              className={linkClass(texts.some(t =>
                pathname.startsWith(`/study/${t.id}`) || pathname.startsWith(`/pariksha/${t.id}`)
              ))}
            >
              Courses
              <svg className="inline-block ml-1 w-3 h-3 opacity-60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {coursesOpen && (
              <div className="absolute left-0 top-full mt-1 w-56 bg-stone-800 border border-stone-700 rounded-lg shadow-xl z-50 py-1">
                {texts.map(text => {
      ...
      <div ref={containerRef} className="relative hidden md:block shrink-0">
        <div className="relative">
          ...
          <input
            ref={inputRef}
            type="text"
            placeholder="Search texts, reference works, and nyāya concepts"
            value={query}
            ...
            {texts.length > 0 && (
              <>
                <button
                  ...
                  className="w-full flex items-center justify-between px-6 py-3 text-sm text-stone-300 hover:bg-stone-800 hover:text-white transition-colors"
                >
                  <span>Courses</span>
                  <svg className={`w-3.5 h-3.5 opacity-60 transition-transform ${mobileCoursesOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {mobileCoursesOpen && texts.map(text => {
                  const href = text.firstPassageId
                    ? `/study/${text.id}/${text.firstPassageId}`
                    : '#'
                  const active =
                    ...
                    pathname.startsWith(`/pariksha/${text.id}`)
                  return (
                    <Link key={text.id} href={href} className={`pl-10 ${mobileLinkClass(active)}`}>
                      {text.title_transliterated}
                    </Link>
                  )
                })}
