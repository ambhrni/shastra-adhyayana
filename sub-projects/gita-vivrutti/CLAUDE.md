...
**Chapter XI (Viśvarūpa-darśana Yoga) — started. 11.1–11.40 done and proofread.**
...
and proofread; Chapter XI is underway (11.1–11.40 done). Continue at śhloka 11.41,
...
Also: a footnote superscript can look like a stray quotation mark in this print — this
has now happened at 11.32 and again (unresolved) at 11.39/11.40 — always zoom in close
on every raised mark near a citation and compare its shape against a confirmed digit
nearby before deciding; don't assume it's punctuation, but don't force a footnote onto
a mark that turns out to be a quote either. 11.41 continues अर्जुन उवाच (no speaker
change expected, but confirm on render) — mūla opens "सखेति मत्वा प्रसभं यदुक्तं...".
```
...
2. Read `gita_vivrutti.html` to see the exact current DEFS dictionary and verse-data structure, and the existing `CHAPTERS[11]` object (verses 1–40 populated so far).
3. Locate 11.41 in the OCR (it follows immediately after 11.40's रा commentary), render and verify against the PDF (page-offset formula: Sanskrit page + 46) — draft, and proceed on approval. Watch for (वि) blocks continuing silently across a page break, as happened at 11.32, and for footnote superscripts that look like stray quotation marks, as happened at 11.32 and again (unresolved both times) at 11.39 and 11.40.
...
*Last updated: Session through śhloka 11.40 (2026-09-20) — Chapter XI (Viśvarūpa-darśana Yoga) continuing; 11.1–11.40 written + proofread. Process refinements, now standing rules (see "Session Workflow" and "Footnotes and Image Citations" sections above):*
...
*Next session: continue Chapter XI at 11.41 — its मूल opens "सखेति मत्वा प्रसभं यदुक्तं..." (already visible in the OCR right after 11.40's रा commentary), continuing अर्जुन उवाच (confirm no speaker change on render). Locate the PDF page (page-offset formula: Sanskrit page + 46; likely PDF 412 or 413), render and verify, draft, and proceed only on explicit approval. Remember: bold only an exact word-form match to the मूल — never a vigraha re-inflection, even one reusing the compound's own roots (see (9) above); footnote numbering resets per page, and can even repeat a digit within one verse's own footnote set if that verse's content spans two pages (see (12)/(13) in the Resume Pointer, and (19) above) — always trace by position, never by digit; watch for व/ब and (ग)/(रा) OCR confusions (see (13) and (16) above), and now also अ/श vs ग्नि-type single-akṣara corruptions (see (21) above); never assume a (वि)/(रा) block's scope ends at a page-image boundary — always render the following page and check for a heading change (see (18) above); **look closely for footnote superscripts that can hide among real quotation marks in राॕ's own commentary text — and when zooming still can't resolve a mark to a confirmed digit, flag it rather than force it either way (see (19) and (21) above)**; re-fetch and verify any file write against the actual machine before reporting it done (see (17) above). Single reference file going forward.*
